...
const AddToken = (props: {
    ...
    const [progress, setProgress] = React.useState<number>(props.initData?.progress || 1);
    const [pageIcon, setPageIcon] = React.useState<string>(props.initData?.icon || '');
    const [target, setTarget] = React.useState<string>(props.initData?.target || '');
    ...
    const storeToken = (encryptedSecret: string, user: string) => {
        chrome.tabs.query({active: true, currentWindow: true}).then((tabs) => {
            if (tabs[0] && tabs[0].id)
        ...
        if (result.startsWith('otpauth://totp/')) {
            try {
                ...
                addToken(secret, pageName, user, algorithm, digits, period).then((encryptedSecret) => {
                    storeToken(encryptedSecret, user);
                    chrome.storage.local.remove('addToken');
                    setProgress(4)
                });
            } catch (e) {
    ...
    return (
        <>
            ...
            {progress === 2 && <div className='flex flex-col mt-2'>
                <span className='text-sm text-white'>Please select the input for the OTP token on the page by clicking on it. Valid elements turn red when hovering over them.</span>
            </div>}
            ...
            {progress === 4 && <div className='flex flex-col mt-2'>
                <span className='text-sm text-white'>Please click on save to store the token.</span>
                <span className='text-sm text-white mt-4'>For future logins, simply click the OTP-Manager extension icon.</span>
            </div>}
